...
void CBodyBasics::Update()
{
	...
	// check for received OSC packets
	ParseOSC();
...
void CBodyBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
	...
	if (m_hWnd)
	{
		...
		if (SUCCEEDED(hr) && m_pRenderTarget && m_pCoordinateMapper)
		{
			...
			// SEND OSC DATA
			TransmitBody(nTime, nBodyCount, ppBodies);
...
HRESULT CBodyBasics::EnsureDirect2DResources()
{
	...
	if (m_pD2DFactory && !m_pRenderTarget)
	{
		...
		// light green
		m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::LightBlue, 1.0f), &m_pBrushJointTracked);

		m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::MediumAquamarine, 1.0f), &m_pBrushJointInferred);
		m_pRenderTarget->CreateSolidColorBrush(D2D1::ColorF(D2D1::ColorF::LightSkyBlue, 1.0f), &m_pBrushBoneTracked);
