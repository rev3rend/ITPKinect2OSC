...
int CBodyBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	// SENDER
	sendsock.connectTo("localhost", SENDPORT);
	if (!sendsock.isOk()) {
		std::string s = "Error connection to port " + std::to_string(SENDPORT) + ": " + sendsock.errorMessage() + "\n";
		printFucker(s);
	}
	else {
		std::string s = "Client started, will send packets to port " + std::to_string(SENDPORT) + "\n";
		printFucker(s);
	}
...
void CBodyBasics::Update()
{
	...
	PacketReader pr;
	PacketWriter pw;
		if (recvsock.isOk() && recvsock.receiveNextPacket(10 /* timeout, in ms */)) {
			...
			while (pr.isOk() && (msg = pr.popMessage()) != 0) {
				...
				if (msg->match("/connect").isOkNoMoreArgs()) {
					...
					std::string s = "Server: received /connect from " + c + "\n";
					printFucker(s);

					// reconnect on new IP address
					sendsock.close();
					sendsock.connectTo(c, SENDPORT);
					if (!sendsock.isOk()) {
						std::string s = "Error connection to port " + std::to_string(SENDPORT) + ": " + sendsock.errorMessage() + "\n";
						printFucker(s);
					}
...
void CBodyBasics::ProcessBody(INT64 nTime, int nBodyCount, IBody** ppBodies)
{
    ...
    if (m_hWnd)
    {
        ...
        if (SUCCEEDED(hr) && m_pRenderTarget && m_pCoordinateMapper && sendsock.isOk())
        {
			...
			ok = sendsock.sendPacket(pw.packetData(), pw.packetSize());
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            ...
                            for (int j = 0; j < _countof(joints); ++j)
                            {
								...
								msg.pushInt32(i);
								msg.pushInt32(j);
								msg.pushFloat(joints[j].Position.X);
								msg.pushFloat(joints[j].Position.Y);
								msg.pushFloat(joints[j].Position.Z);
								// send message
								pw.init();
								pw.startBundle().startBundle().addMessage(msg).endBundle().endBundle();
								ok = sendsock.sendPacket(pw.packetData(), pw.packetSize());
								...
								//	//OutputDebugStringW(L"left hand!!!\n");
								//	std::string s = "left hand: " + std::to_string(joints[j].Position.X) + "\n";
								//	printFucker(s);
...
void CBodyBasics::printFucker(std::string s)
{
